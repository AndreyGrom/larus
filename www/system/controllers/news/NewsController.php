...
class NewsController extends Controller {
    ...
    public function Index(){
        ...
        elseif ($this->GetAliasFromQuery()!==''){ // если есть алиас
            ...
            $sql = "SELECT * FROM `".db_pref."news_c` WHERE `ALIAS` = '$this->alias' AND `PUBLIC`=1 LIMIT 1";
            ...
            $this->page_title = $this->config->NewsModuleTitle;
            $this->meta_title = $this->config->NewsModuleTitle.' - '. $this->config->SiteTitle;
            $this->meta_description = $this->config->NewsModuleDescription;
            $this->meta_keywords = $this->config->NewsModuleKeywords;
    ...
    public function ShowCategories(){
        $sql = "SELECT * FROM `".db_pref."news_c` WHERE `PARENT` = '$this->cid' AND `PUBLIC`=1"; // получаем потомков
        $query = $this->db->query($sql);
        if ($this->db->num_rows($query) > 0){ // Если в категории есть подкатегории, то выводим их
            for ($i=0; $i < $this->db->num_rows($query); $i++) {
                $row = $this->db->fetch_array($query);
                if ($row['IMAGE']!==''){
                    $row['IMAGE_NEW'] = Func::getInstance()->GetImage(UPLOAD_IMAGES_DIR.'news/'.$row['IMAGE'], $this->config->NewsImageWidthCategoryList,$this->config->NewsImageHeightCategoryList,'','news');
                }
    ...
    public function ShowItems(){
        ...
        if($this->cid > 0 || isset($this->get['tag'])){
            ...
            $this->breadcrumbs[] = array('text' => $this->config->NewsModuleTitle, 'href' => '/news');
            $this->breadcrumbs[] = array('text' => $this->page_title, 'href' => '');
        } else{
            $this->breadcrumbs[] = array('text' => $this->config->NewsModuleTitle, 'href' => '');
        }
        if (isset($this->get['tag'])){
            $sql = "SELECT *, @id:=ID, (SELECT COUNT(*) FROM `".db_pref."comments` WHERE `CONTROLLER` = 'news' AND `MATERIAL_ID` = @id) AS COMMENTS_COUNT FROM `".db_pref."news_i`  WHERE `TAGS` LIKE '%,$tag,%' AND `PUBLIC`=1 ORDER BY DATE_PUBL DESC";

        }
        elseif($this->cid==0){
            $sql = "SELECT *, @id:=ID, (SELECT COUNT(*) FROM `".db_pref."comments` WHERE `CONTROLLER` = 'news' AND `MATERIAL_ID` = @id) AS COMMENTS_COUNT FROM `".db_pref."news_i`  WHERE `PUBLIC`=1 ORDER BY DATE_PUBL DESC";
        } else {
            $sql = "SELECT *, @id:=ID, (SELECT COUNT(*) FROM `".db_pref."comments` WHERE `CONTROLLER` = 'news' AND `MATERIAL_ID` = @id) AS COMMENTS_COUNT FROM `".db_pref."news_i`  WHERE `PARENT` LIKE '%,$this->cid,%' AND `PUBLIC`=1 ORDER BY DATE_PUBL DESC";
            $url .=$this->alias.'/';
        }
        ...
        $params = array(
            'sql' => $sql,
            'per_page' => $this->config->NewsItemListPerPage,
        ...
        if ($items){
            foreach ($items as &$item){
                ...
                if ($item['SKIN']!==''){
                    $item['SKIN_NEW'] = Func::getInstance()->GetImage(UPLOAD_IMAGES_DIR.'news/'.$item['SKIN'], $this->config->NewsImageWidthItemList,$this->config->NewsImageHeightItemList,'','news');
                }
            }
    ...
    public function ShowItem($id=0){;
        ...
        if ($this->db->num_rows($query) > 0){
            ...
            sort($parents);
            $str = implode(",",$parents);
            $sql = "SELECT * FROM `".db_pref."news_c`  WHERE `ID` IN ($str) AND `PUBLIC`=1";
            $query = $this->db->query($sql);
            for ($i=0; $i < $this->db->num_rows($query); $i++) {
                $row = $this->db->fetch_array($query);
                $categories[] = $row;
            ...
            if ($this->config->NewsCommentsEnabled && $item['COMMENTS']){
                ...
                $comments->premoderation = $this->config->NewsCommentsModerationEnabled;
                $comments->captcha = $this->config->NewsCommentsCaptchaEnabled;
            ...
            $this->breadcrumbs = array(
                ...
                array('text' => $this->config->NewsModuleTitle, 'href' => '/news'),
                array('text' => $categories[0]['TITLE'], 'href' => '/news/'.$categories[0]['ALIAS']),
            ...
            $this->assign(array(
                ...
                'image'            => $item['SKIN'],
                'page_title'       => $this->page_title ,
                'image_new'        => Func::getInstance()->GetImage(UPLOAD_IMAGES_DIR.'news/'.$item['SKIN'], $this->config->NewsImageWidthItem,$this->config->NewsImageHeightItem,'','news'),
                'images'           => explode(",",$item['IMAGES']),
            ));

            $this->SetPath('news/single/');
