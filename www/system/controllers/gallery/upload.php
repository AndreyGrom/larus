...
if (isset($_FILES)){
    ...
    if (isset($_FILES["image"])){
        $id = $_GET['id'];
        $images = array();
        $upload = $_SERVER['DOCUMENT_ROOT'].'/'.UPLOAD_IMAGES_DIR.'gallery/';
        $cache_img = $_SERVER['DOCUMENT_ROOT'].'/'.CACHE_IMAGES;
        $sql = "SELECT * FROM `".db_pref."gallery_i` WHERE ID=$id";
        $query = $db->query($sql);
        ...
        if (is_uploaded_file($_FILES["image"]["tmp_name"])) {
            ...
            $str = implode(",", $images);
            $sql = "UPDATE `".db_pref."gallery_i` SET `IMAGES`='$str' $skin WHERE ID=$id";
            $query = $db->query($sql);
        }
    ...
    if (isset($_FILES["file"])){
        $id = $_GET['id'];
        $files = array();
        $upload = $_SERVER['DOCUMENT_ROOT'].'/'.UPLOAD_FILES_DIR.'gallery/';
        $sql = "SELECT * FROM `".db_pref."gallery_i` WHERE ID=$id";
        $query = $db->query($sql);
        $row = $db->fetch_array($query);
        $str = $row["FILES"];
        ...
        if (is_uploaded_file($_FILES["file"]["tmp_name"])) {
            ...
            $files[] = $file_info;
            $str = serialize($files);
            $sql = "UPDATE `".db_pref."gallery_i` SET `FILES`='$str' WHERE ID=$id";
            $query = $db->query($sql);
        }
        $smarty->assign(array(
            'file_item' => $file_info,
...
if (isset($_POST['skin'])){
    $id = $_POST['id'];
    $skin = $_POST['skin'];
    $sql = "UPDATE `".db_pref."gallery_i` SET `SKIN`='$skin' WHERE ID=$id";
    $db->query($sql);
}
if (isset($_POST['image'])){
    ...
    if ($str!==''){
        ...
        $str = implode(',',$images_);
        $sql = "UPDATE `".db_pref."gallery_i` SET `IMAGES`='$str' WHERE ID=$id";
        $db->query($sql);

        $file = $_SERVER['DOCUMENT_ROOT'].'/'.UPLOAD_IMAGES_DIR.'gallery/'.$image;
        if (file_exists($file)){
            unlink($file);
        }
    }
...
if (isset($_POST['file'])){
    ...
    if ($str!==''){
        ...
        $file = $_SERVER['DOCUMENT_ROOT'].'/'.UPLOAD_FILES_DIR.'gallery/'.$file;
        if (file_exists($file)){
            unlink($file);
        }
    }
