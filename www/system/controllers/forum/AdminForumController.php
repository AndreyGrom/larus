...
/*        $this->js[]   = HTML_PLUGINS_DIR.'cleeditor/jquery.cleditor.min.js';
    ...
    public function SaveSettings(){
        ...
        $this->Head("?c=forum&act=settings");
    }
    public function SaveCategory(){
        ...
        $upload_path = UPLOAD_IMAGES_DIR.'forum/';
        $image = Func::getInstance()->UploadFile($_FILES["image"]['name'],$_FILES["image"]['tmp_name'], $upload_path);
        if ($this->act == 'new_c'){
            $sql = "
        INSERT INTO `".db_pref."forum_c` (
        `PARENT`,`TITLE`, `DESC`, `META_DESC`,`META_KEYWORDS`, `PUBLIC`,`TEMPLATE`, `POSITION`, `IMAGE`)
        VALUES
        ('$parent','$title','$desc','$meta_desc','$meta_keywords','$publ','$template','99999','$image')";
            ...
            $sql = "UPDATE `".db_pref."forum_c` SET
            `PARENT` = '$parent',
            `TITLE` = '$title',
            `DESC` = '$desc',
            `META_DESC` = '$meta_desc',
        ...
        $this->Head("?c=forum&cid=$this->cid");
    }
    ...
    public function SaveItem(){
        ...
        $date = time();
        $sql = "SELECT * FROM `".db_pref."forum_i` WHERE `ALIAS`='$alias'";
        if (isset($this->id)){
            $sql .= " AND ID <> $this->id";
        }
        $query = $this->db->query($sql);
        ...
        `FILE3_NAME` = '$file3_name'
         WHERE `ID` = $this->id";
            ...
            $this->Head("?c=forum&cid=".$parents[0]."&id=$this->id");
        } else {
    ...
    public function getCategories($parent = false){
        $categories = array();
        $sql = "SELECT * FROM `".db_pref."forum_c` ORDER BY `ID`";
        $query = $this->db->query($sql);
    ...
    public function getPagination($parent = false){
        ...
        $sql = "SELECT * FROM `".db_pref."forum_i` $where";
        $query = $this->db->query($sql);
    ...
    public function getItems($parent = false, $start, $count){
        ...
        $sql = "SELECT * FROM `".db_pref."forum_i` $where ORDER BY `ID` DESC $limit";
        $query = $this->db->query($sql);
        for ($i=0; $i < $this->db->num_rows($query); $i++) {
            $row = $this->db->fetch_array($query);
    ...
    public function getCategory($cid){
        $row = array();
        $sql = "SELECT * FROM `".db_pref."forum_c` WHERE `ID`=$cid LIMIT 1";
        $query = $this->db->query($sql);
        if ($this->db->num_rows($query)>0){
            ...
            $row['DATE_EDIT'] = $this->DateFormat($row["DATE_EDIT"]);
            if ($row['IMAGE']!==''){
                $row['NEW_IMAGE'] = Func::getInstance()->GetImage(UPLOAD_IMAGES_DIR.'forum/'.$row['IMAGE'], 50,50,'','forum');
            }
        }
        return $row;
    }
    public function getItem($id){
        $sql = "SELECT i.*, c.ID AS CID, c.TITLE AS CAT_NAME, c.ALIAS AS CAT_ALIAS FROM `".db_pref."forum_i` i LEFT JOIN `".db_pref."forum_c` c ON i.PARENT=c.ID WHERE i.`ID`=$id LIMIT 1";
        $query = $this->db->query($sql);
        $row = $this->db->fetch_array($query);
        $row["DATE_PUBL"] = $this->DateFormat($row["DATE_PUBL"]);
        $row["DATE_EDIT"] = $this->DateFormat($row["DATE_EDIT"]);
        ...
        if ($row['IMAGES']!==''){
            ...
            foreach ($images as $img){
                $new_images[] = $this->func->GetImage(UPLOAD_IMAGES_DIR.'forum/'.$img, 100,100,'','forum');
            }
        }
        $row['IMAGES'] = $images;
    ...
    public function ShowNewCat(){
        $this->assign(array(
            'templates'      => $this->func->getTemplates($this->templates_dir.'forum/list/'),
    ...
    public function ShowNewItem(){
        $this->assign(array(
            'templates' => $this->func->getTemplates($this->templates_dir.'forum/items/'),
            'categories' => $this->structure,
            'new'       => true,
        ));
    ...
    public function DeleteCategory($id){
        $sql = "SELECT * FROM `".db_pref."forum_i` WHERE `PARENT` LIKE '%,$id,%'";
        $query = $this->db->query($sql);
        if ($this->db->num_rows($query) == 0){
            $sql = "DELETE FROM `".db_pref."forum_c` WHERE `ID`=$id";
            $query = $this->db->query($sql);
            $this->Head('?c=forum');
        } else {
            $_SESSION['alert'] = 'Сначала удалите материалы из этой категории!';
            $this->Head("?c=forum&cid=$id");
        }
    }
    public function DeleteItem($id){
        $sql = "SELECT * FROM `".db_pref."forum_i` WHERE `ID`=$id LIMIT 1";
        $query = $this->db->query($sql);
        $row = $this->db->fetch_array($query);
        if ($row['IMAGES']!==''){
            $images = explode(",", $row['IMAGES']);
            foreach ($images as $img){
                if (file_exists(UPLOAD_IMAGES_DIR.'forum/'.$img)){
                    unlink(UPLOAD_IMAGES_DIR.'forum/'.$img);
                }
            }
        }
    ...
    public function ShowCategory($row){
        $this->assign(array(
            ...
            'category_image'           => UPLOAD_IMAGES_DIR.'forum/'.$row['IMAGE'],
            'category'                 => $row,
            'categories'               => $this->categories,
            'templates'                => Func::getInstance()->getTemplates($this->templates_dir.'forum/list/'),
        ));
        $this->content = $this->SetTemplate('cat.tpl');
    }
    public function ShowItem($row){
        $this->assign(array(
            ...
            'item_id'                  => $row['ID'],
            'categories'               => $this->structure,
            'templates'                => Func::getInstance()->getTemplates($this->templates_dir.'forum/items/'),
            'cid'                      => $row['CID'],
            'c_name'                   => $row['CAT_NAME'],
