...
class AdminPartnersController extends AdminController {
    ...
    public function SaveFields(){
        ...
        $this->Head("?c=partners&act=fields");
    }
    public function SaveSettings(){
        ...
        $this->Head("?c=partners&act=settings");
    }
    public function SaveCategory(){
        ...
        $sql = "SELECT * FROM `".db_pref."partners_c` WHERE `ALIAS`='$alias'";
        if ($this->act!=='new_c'){
            $sql .= " AND ID <> $this->cid";
        }
        $query = $this->db->query($sql);
        if ($this->db->num_rows($query) == 0){
            $upload_path = UPLOAD_IMAGES_DIR.'partners/';
            $image = Func::getInstance()->UploadFile($_FILES["image"]['name'],$_FILES["image"]['tmp_name'], $upload_path);

            if ($this->act == 'new_c'){
                $sql = "
                    INSERT INTO `".db_pref."partners_c` (
                ...
                $sql = "UPDATE `".db_pref."partners_c` SET
                `PARENT` = '$parent',
                `TITLE` = '$title',
                `DESC` = '$desc',
                `DESC2` = '$desc2',
            ...
            $this->Head("?c=partners&cid=$this->cid");
        } else {
    ...
    public function SaveItem(){
        ...
        $sql = "SELECT * FROM `".db_pref."partners_i` WHERE `ALIAS`='$alias'";
        if (isset($this->id)){
            $sql .= " AND ID <> $this->id";
        }
        $query = $this->db->query($sql);
        if ($this->db->num_rows($query) == 0){ // если такого алиаса нет
            if ($this->act == 'new'){
                $this->db->insert('agcms_partners_i', $params);
                $this->id = $this->db->last_id();

            }else{
                $this->db->update('agcms_partners_i', $params, "ID = " . $this->id);
            }
            $this->Head("?c=partners&cid=".$parents[0]."&id=$this->id");
        } else {
            $this->session['alert'] = 'Такой алиас уже существует';
        }
    }
    ...
    public function getCategories($parent = false){
        $categories = array();
        $sql = "SELECT * FROM `".db_pref."partners_c` ORDER BY `ID`";
        $query = $this->db->query($sql);
    ...
    public function getPagination($parent = false){
        ...
        $sql = "SELECT * FROM `".db_pref."partners_i` $where";
        $query = $this->db->query($sql);
    ...
    public function getItems($parent = false, $start, $count){
        ...
        $sql = "SELECT * FROM `".db_pref."partners_i` $where ORDER BY `ID` DESC $limit";
        $query = $this->db->query($sql);
        for ($i=0; $i < $this->db->num_rows($query); $i++) {
            $row = $this->db->fetch_array($query);
    ...
    public function getCategory($cid){
        $row = array();
        $sql = "SELECT * FROM `".db_pref."partners_c` WHERE `ID`=$cid LIMIT 1";
        $query = $this->db->query($sql);
        if ($this->db->num_rows($query)>0){
            $row = $this->db->fetch_array($query);
            $row['DATE_PUBL'] = $this->DateFormat($row["DATE_PUBL"]);
            $row['DATE_EDIT'] = $this->DateFormat($row["DATE_EDIT"]);
            if ($row['IMAGE']!==''){
                $row['NEW_IMAGE'] = Func::getInstance()->GetImage(UPLOAD_IMAGES_DIR.'partners/'.$row['IMAGE'], 50,50,'','partners');
            }
        }
        return $row;
    }
    ...
    public function getItem($id){
        $sql = "SELECT i.*, c.ID AS CID, c.TITLE AS CAT_NAME, c.ALIAS AS CAT_ALIAS FROM `".db_pref."partners_i` i LEFT JOIN `".db_pref."partners_c` c ON i.PARENT=c.ID WHERE i.`ID`=$id LIMIT 1";
        $query = $this->db->query($sql);
        $row = $this->db->fetch_array($query);
        ...
        if ($row['IMAGES']!==''){
            ...
            foreach ($images as $img){
                $new_images[] = $this->func->GetImage(UPLOAD_IMAGES_DIR.'partners/'.$img, 100,100,'','partners');
            }
        }
        $row['IMAGES'] = $images;
    ...
    public function ShowNewCat(){
        $this->assign(array(
            'templates'      => $this->func->getTemplates($this->templates_dir.'partners/list/'),
    ...
    public function ShowNewItem(){
        $this->assign(array(
            'templates' => $this->func->getTemplates($this->templates_dir.'partners/single/'),
            'categories' => $this->structure,
            'new'       => true,
    ...
    public function DeleteCategory($id){
        $sql = "SELECT * FROM `".db_pref."partners_i` WHERE `PARENT` LIKE '%,$id,%'";
        $query = $this->db->query($sql);
        if ($this->db->num_rows($query) == 0){
            $sql = "DELETE FROM `".db_pref."partners_c` WHERE `ID`=$id";
            $query = $this->db->query($sql);
            $this->Head('?c=partners');
        } else {
            $_SESSION['alert'] = 'Сначала удалите материалы из этой категории!';
            $this->Head("?c=partners&cid=$id");
        }
    }
    public function DeleteItem($id){
        $sql = "SELECT * FROM `".db_pref."partners_i` WHERE `ID`=$id LIMIT 1";
        $query = $this->db->query($sql);
        $row = $this->db->fetch_array($query);
        if ($row['IMAGES']!==''){
            $images = explode(",", $row['IMAGES']);
            foreach ($images as $img){
                if (file_exists(UPLOAD_IMAGES_DIR.'partners/'.$img)){
                    unlink(UPLOAD_IMAGES_DIR.'partners/'.$img);
                }
            }
        }
        $sql = "DELETE FROM `".db_pref."partners_i` WHERE `ID`=$id";
        $query = $this->db->query($sql);
    }
    public function ShowCategory($row){
        $this->assign(array(
            ...
            'category_image'           => UPLOAD_IMAGES_DIR.'partners/'.$row['IMAGE'],
            'category'                 => $row,
            'categories'               => $this->categories,
            'templates'                => Func::getInstance()->getTemplates($this->templates_dir.'partners/list/'),
        ));
        $this->content = $this->SetTemplate('cat.tpl');
    }
    public function ShowItem($row){
        $this->assign(array(
            ...
            'item_id'                  => $row['ID'],
            'categories'               => $this->structure,
            'templates'                => Func::getInstance()->getTemplates($this->templates_dir.'partners/single/'),
            'cid'                      => $row['CID'],
            'c_name'                   => $row['CAT_NAME'],
    ...
    public function getSiteMap(){
        $sql = "SELECT ALIAS FROM `".db_pref."partners_i`  WHERE `PUBLIC`=1 ORDER BY `DATE_PUBL` DESC";
        ...
        if ($this->db->num_rows($result)){
            for ($i = 0; $i < $this->db->num_rows($result); $i++){
                ...
                $return[]  = array(
                    'loc'           => $this->site_url . 'partners/' . $row['ALIAS'] .'/',
                    'changefreq'    => 'weekly',
                    'priority'      => '1',
                );
            }
    ...
    public function Index(){
        ...
        if(count($this->categories) == 0 && !isset($this->act)){
            $this->head('?c=partners&act=new_c');
        }
