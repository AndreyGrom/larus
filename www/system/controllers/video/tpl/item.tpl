            ...
            <h3 class="panel-title text-uppercase">
                ...
                {if !$new}
                    <span class="pull-right text-lowercase">
                            <a href="?c=video&act=new" class="btn btn-info btn-xs" data-toggle="tooltip" title="" data-original-title="Создать новвый материал"><span class="glyphicon glyphicon-copy"></span></a>
                            <a href="?c=video&act=del-item&id={$item_id}&cid={$item_parents[0]}" class="btn btn-danger btn-xs confirm" data-toggle="tooltip" title="" data-original-title="Удалить материал"><span class="glyphicon glyphicon-remove"></span></a>
                    </span>
                {/if}
            ...
            {if !$new}
                <div class="alert alert-info">
                    ...
                    <div class="row">
                        ...
                        <span class="col-sm-9"><a target="_blank" href="{$site_url}video/{$item_alias}">{$site_url}video/{$item_alias}</a></span>
                    </div>
                    {/if}
                    <div class="row">
                        <span class="col-sm-3 text-right">Короткая ссылка:</span>
                        <span class="col-sm-9"><a target="_blank" href="{$site_url}video/{$item_id}">{$site_url}video/{$item_id}</a></span>
                    </div>
                </div>
                {if $item_id}
                <div class="alert alert-info">
                    <p>Код вызова: <input onclick="this.select()" class="form-control" style="display:inline-block;width: 270px" type="text" readonly value='[widjet name="vid" params="{$item_id}"]'/></p>
                </div>
                {/if}
            {/if}
            <div class="tabs">
                ...
                <div class="tab-content">
                    ...
                    <div id="tab-video" class="tab-pane fade">
                        <h3></h3>
                        <p>{include file="item-video.tpl"}</p>
                    </div>
...
/*        $.cleditor.set_rfm('/filemanager');
    ...
    $("#item-form").submit(function(){
        ...
        if (chs.length==0){
            alert("Необходимо выбрать хотя бы один канал");
            return false;
        }
    });
